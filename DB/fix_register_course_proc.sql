-- Sửa stored procedure để cho phép đăng ký khóa học mà không bắt buộc phải có lớp đang tuyển sinh
-- Lớp học sẽ được gán sau khi nhân viên duyệt đơn
...
BEGIN
   ...
   -- Tìm lớp học thuộc khóa này đang tuyển sinh (nếu có)
   -- KHÔNG BẮT BUỘC - Cho phép đăng ký mà chưa có lớp
   BEGIN
      SELECT id_lop_hoc
        INTO v_lop_id
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         -- Không có lớp đang tuyển sinh -> Để NULL, nhân viên sẽ gán sau
         v_lop_id := NULL;
   END;

   -- Kiểm tra xem học viên đã đăng ký khóa này chưa (tránh trùng)
   DECLARE
   ...
   VALUES (
      ...
      N'Chờ duyệt',
      v_hoc_vien_id,
      v_lop_id  -- Có thể NULL nếu chưa có lớp
   );
