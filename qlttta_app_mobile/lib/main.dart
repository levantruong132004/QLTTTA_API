...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    super.initState();
    // Bắt đầu monitor phiên làm việc (poll mỗi 5s)
    SessionMonitor().start();
    // Lắng nghe sự kiện unauthorized để tự điều hướng về Login
    _authStream = AuthManager().stream;
    _authStream.listen((evt) async {
      if (evt.type == 'unauthorized') {
        // Xoá session local ngay để monitor không còn poll và gây cản trở đăng nhập lại
        try {
