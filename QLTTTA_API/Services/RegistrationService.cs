...
namespace QLTTTA_API.Services
{
    ...
    public class RegistrationService : BaseService, IRegistrationService
    {
        ...
        public async Task<List<Registration>> GetRegistrationsAsync(string? status = null, int? classId = null, string? classCode = null)
        {
            ...
            {
                // Dùng LIKE + TRIM để bao quát các biến thể 'Chờ duyệt', 'Chờ duyệt ', 'Chờ duyệt - ghi chú'
                where.Add("UPPER(TRIM(dk.TRANG_THAI)) LIKE UPPER(TRIM(:st)) || '%' ");
            }
            if (classId.HasValue) where.Add("dk.ID_LOP_HOC = :cid");
            if (!string.IsNullOrWhiteSpace(classCode)) where.Add("UPPER(lh.MA_LOP_HOC) = UPPER(:ccode)");
            ...
            var sql = $@"SELECT dk.ID_DANG_KY AS REGISTRATION_ID,
                           ...
                           JOIN QLTT_ADMIN.KHOA_HOC kh ON kh.ID_KHOA_HOC = lh.ID_KHOA_HOC{whereSql}
                            ORDER BY dk.ID_DANG_KY DESC";

            using var cmd = new OracleCommand(sql, conn) { BindByName = true };
            if (!string.IsNullOrWhiteSpace(status)) cmd.Parameters.Add(":st", OracleDbType.NVarchar2).Value = status;
            if (classId.HasValue) cmd.Parameters.Add(":cid", OracleDbType.Int32).Value = classId.Value;
            if (!string.IsNullOrWhiteSpace(classCode)) cmd.Parameters.Add(":ccode", OracleDbType.Varchar2).Value = classCode;
        ...
        public async Task<ApiResponse<bool>> ApproveAsync(int registrationId, int? newClassId = null)
        {
            try
            {
                ...
                int classIdToUse = 0;
                using (var getReg = new OracleCommand("SELECT MA_DANG_KY, ID_LOP_HOC FROM QLTT_ADMIN.DON_DANG_KY WHERE ID_DANG_KY = :id", conn))
                {
                ...
                {
                    using var upClass = new OracleCommand("UPDATE QLTT_ADMIN.DON_DANG_KY SET ID_LOP_HOC = :cid WHERE ID_DANG_KY = :id", conn) { BindByName = true };
                ...
                using (var cmdCnt = new OracleCommand("SELECT (SELECT COUNT(*) FROM QLTT_ADMIN.DON_DANG_KY WHERE ID_LOP_HOC=:cid AND TRANG_THAI = N'Đã duyệt') AS CNT, (SELECT SI_SO_TOI_DA FROM QLTT_ADMIN.LOP_HOC WHERE ID_LOP_HOC=:cid) AS MAXS FROM DUAL", conn))
                {
                ...
                // Lấy ID nhân viên bằng kết nối Admin để tránh lỗi quyền (ORA-00942)
                ...
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sid))
                    {
                        ...
                        using var findStaff = new OracleCommand("SELECT ID_NGUOI_DUNG FROM QLTT_ADMIN.TAI_KHOAN WHERE SESSION_ID_PC = :sid OR SESSION_ID_MOBILE = :sid", adminConn) { BindByName = true };
                        findStaff.Parameters.Add(":sid", OracleDbType.Varchar2).Value = sid;
                        var o = await findStaff.ExecuteScalarAsync();
                        if (o != null && o != DBNull.Value)
                        {
                            var staffId = Convert.ToInt32(o);
                            using var chk = new OracleCommand("SELECT COUNT(*) FROM QLTT_ADMIN.NHAN_VIEN_HOC_VU WHERE ID_NHAN_VIEN = :id", adminConn) { BindByName = true };
                ...
                {
                    _logger.LogWarning(ex, "Could not resolve staff ID via Admin connection");
                }

                // Duyệt đơn trực tiếp (không gọi SP) để tránh thiếu quyền EXECUTE
                using (var up = new OracleCommand(@"UPDATE QLTT_ADMIN.DON_DANG_KY
                ...
                using (var cmdRecheck = new OracleCommand("SELECT (SELECT COUNT(*) FROM QLTT_ADMIN.DON_DANG_KY WHERE ID_LOP_HOC=:cid AND TRANG_THAI = N'Đã duyệt') AS CNT, (SELECT SI_SO_TOI_DA FROM QLTT_ADMIN.LOP_HOC WHERE ID_LOP_HOC=:cid) AS MAXS FROM DUAL", conn))
                {
                    ...
                    if (await rdr2.ReadAsync())
                    {
                        ...
                        if (cnt2 >= max2)
                        {
                            using var upClassStatus = new OracleCommand("UPDATE QLTT_ADMIN.LOP_HOC SET TRANG_THAI = N'Đã đủ sĩ số' WHERE ID_LOP_HOC = :cid AND TRANG_THAI <> N'Đã đủ sĩ số'", conn);
        ...
        {
            using var conn = await GetConnectionAsync();
            // Xác định học viên theo USER của kết nối per-user
            int hvId = 0;
            using (var cmd = new OracleCommand("SELECT ID_NGUOI_DUNG FROM QLTT_ADMIN.TAI_KHOAN WHERE UPPER(TEN_DANG_NHAP)=USER", conn))
            {
                var scalar = await cmd.ExecuteScalarAsync();
                if (scalar == null || scalar == DBNull.Value) return new List<Registration>();
        ...
        // Tìm kiếm đơn theo id/mã đơn/mã lớp/mã khóa (phục vụ quét QR)
        public async Task<List<Registration>> SearchAsync(string q)
        {
            using var conn = await GetConnectionAsync();
            int? id = null;
            if (int.TryParse(q, out var parsed)) id = parsed;
            var kw = $"%{q?.Trim().ToUpperInvariant()}%";
            ...
            var sql = @"SELECT dk.ID_DANG_KY AS REGISTRATION_ID,
                                 ...
                                 kh.TEN_KHOA_HOC AS TEN_KHOA_HOC
                                     ...
                                     JOIN QLTT_ADMIN.LOP_HOC lh ON lh.ID_LOP_HOC = dk.ID_LOP_HOC
                                     JOIN QLTT_ADMIN.KHOA_HOC kh ON kh.ID_KHOA_HOC = lh.ID_KHOA_HOC
                           WHERE ( :id IS NOT NULL AND dk.ID_DANG_KY = :id )
                              OR ( UPPER(dk.MA_DANG_KY) LIKE :kw )
                              OR ( UPPER(lh.MA_LOP_HOC) LIKE :kw )
                              OR ( UPPER(kh.MA_KHOA_HOC) LIKE :kw )
                           ORDER BY dk.ID_DANG_KY DESC";

            using var cmd = new OracleCommand(sql, conn) { BindByName = true };
            cmd.Parameters.Add(":id", OracleDbType.Int32).Value = (object?)id ?? DBNull.Value;
            cmd.Parameters.Add(":kw", OracleDbType.Varchar2).Value = kw;
            var list = new List<Registration>();
            using var r = await cmd.ExecuteReaderAsync();
            while (await r.ReadAsync())
            {
                list.Add(new Registration
                {
                    ...
                    RegistrationDate = r.IsDBNull(2) ? null : r.GetDateTime(2),
                    Status = r.IsDBNull(3) ? null : r.GetString(3),
                    StudyDate = r.IsDBNull(4) ? null : r.GetDateTime(4),
                    StudentId = r.GetInt32(5),
                    ClassId = r.GetInt32(6),
                    StaffId = r.GetInt32(7),
                    ClassName = r.IsDBNull(8) ? null : r.GetString(8),
        ...
        public async Task<List<AccountantRegDetail>> GetAccountantRegistrationsAsync(int? courseId = null, int? classId = null)
        {
            ...
            var sql = $@"SELECT dk.ID_DANG_KY,
                          ...
                          LEFT JOIN QLTT_ADMIN.HOA_DON hd ON hd.ID_DANG_KY = dk.ID_DANG_KY{whereSql}
                          ORDER BY dk.ID_DANG_KY DESC";
            object? p = null;
            ...
            // Set CLIENT_IDENTIFIER = username of current requester so VPD on HOC_VIEN recognizes KeToan role
            // Set CLIENT_IDENTIFIER = username of current requester so VPD on HOC_VIEN recognizes KeToan role
            /*
            try
            {
                var headers = _httpContextAccessor.HttpContext?.Request?.Headers;
                var sid = headers?["X-Session-Id"].FirstOrDefault();
                ...
                if (!string.IsNullOrWhiteSpace(sid))
                {
                    var sessionCol = deviceType == "mobile" ? "SESSION_ID_MOBILE" : "SESSION_ID_PC";
                    string? uname = null;
                    using (var findU = new OracleCommand($"SELECT TEN_DANG_NHAP FROM QLTT_ADMIN.TAI_KHOAN WHERE {sessionCol} = :sid", conn) { BindByName = true })
                    {
                        findU.Parameters.Add(":sid", OracleDbType.Varchar2).Value = sid;
                    ...
                    if (!string.IsNullOrWhiteSpace(uname))
                    {
                        ...
                        setId.Parameters.Add(":u", OracleDbType.Varchar2).Value = uname;
                        setId.Parameters.Add(":i", OracleDbType.Varchar2).Value = $"role=accountant;user={uname}";
