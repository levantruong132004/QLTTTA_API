...
namespace QLTTTA_WEB.Controllers
{
    public class StudentController : Controller
    {
                        ...
                        if (invSigRes.IsSuccessStatusCode)
                        {
                            ...
                            if (TryGetPropertyCI(sigDoc.RootElement, "data", out var data))
                            {
                                ...
                                if (TryGetPropertyCI(data, "signature", out var sigData) && sigData.ValueKind != JsonValueKind.Null)
                                {
                                    ...
                                    {
                                        IsSigned = true,
                                        IsValid = TryGetPropertyCI(sigData, "isValid", out var iv) && iv.ValueKind == JsonValueKind.True,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ConfirmPay(int registrationId, int invoiceId, string paymentMethod)
        {
            if (HttpContext.Session.GetString("UserId") == null)
                return RedirectToAction("Login", "Account");
            ...
            try
            {
                var payload = new { InvoiceId = invoiceId, PaymentMethod = paymentMethod };
                var content = new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json");
                var res = await _httpClient.PostAsync("api/invoices/student-pay", content);
                var body = await res.Content.ReadAsStringAsync();
                if (res.IsSuccessStatusCode)
                {
                    TempData["SuccessMessage"] = "Đã ghi nhận yêu cầu thanh toán. Vui lòng chờ xác nhận.";
                }
                else
                {
                    string msg = "Không thể khởi tạo thanh toán";
                    try
                    {
